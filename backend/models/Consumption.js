...
class Consumption {
  ...
  // FIKK: class_code datang berupa nama ruangan penuh (mis. "U5.02.01"), bukan
  // kode 2-digit legacy Psikologi. normalizeClassId.slice(-2) di atas salah
  // untuk kasus ini (semua ruangan FIKK ke-mapping jadi "01" karena semuanya
  // diakhiri "01","02", dst dari suffix nama, bukan dari classes.id asli) --
  // jadi resolve dulu ke classes.id sungguhan lewat lookup nama, baru di-pad.
  static normalizeOccupancy(value) {
    if (value === undefined || value === null || value === '') {
      return null;
    }
    if (typeof value === 'string') {
      const v = value.toLowerCase();
      if (['occupied', 'on', '1', 'true', 'yes'].includes(v)) return 1;
      if (['vacant', 'off', '0', 'false', 'no'].includes(v)) return 0;
    }
    return Consumption.normalizeNumber(value);
  }

  static async resolveClassId(rawValue) {
    ...
    // Kode legacy 1-2 digit (mis. "02") -> pakai apa adanya, tidak perlu lookup.
    if (stringValue.length <= 2) {
      return Consumption.normalizeClassId(stringValue);
    }
    try {
      const [rows] = await db.query('SELECT id FROM classes WHERE name = ? LIMIT 1', [stringValue]);
      if (rows && rows[0]) {
        return Consumption.normalizeClassId(String(rows[0].id));
      }
    } catch (_) {
      // classes table mungkin tidak ada di tenant ini -- jatuh ke fallback di bawah
    }
    // Tidak ketemu di classes -- fallback ke perilaku lama (bukan ideal, tapi
  ...
  // resolvedClassId: hasil dari resolveClassId() -- sudah di-lookup ke
  // classes.id sungguhan (async), jadi normalizeRow sendiri tetap sync.
  static normalizeRow(data = {}, resolvedClassId) {
    const now = new Date();
    ...
    return {
      ...
      id_class: resolvedClassId !== undefined
        ? resolvedClassId
        : Consumption.normalizeClassId(data.id_class ?? data.class_code ?? data.classCode),
      occupancy: Consumption.normalizeOccupancy(data.occupancy),
      power_ac: powerAc,
  ...
  static async create(data) {
    try {
      const resolvedClassId = data.id_class !== undefined && data.id_class !== null
        ? Consumption.normalizeClassId(data.id_class)
        : await Consumption.resolveClassId(data.class_code ?? data.classCode);
      const row = Consumption.normalizeRow(data, resolvedClassId);
